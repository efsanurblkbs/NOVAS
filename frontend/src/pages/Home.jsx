        ...
        <div className="flex gap-4 mb-8">
          <button
            ...
            className={`px-6 py-2.5 rounded-full font-black uppercase text-[10px] tracking-widest transition-all flex items-center gap-2 ${
              activeTab === "explore" ? "bg-[#FFB347] text-white shadow-lg shadow-[#FFB347]/30 scale-105" : "bg-white text-slate-400 hover:bg-slate-50 border border-slate-100"
            }`}
          >
            <Map size={14} /> Keşfet
          </button>
          <button
            ...
            className={`px-6 py-2.5 rounded-full font-black uppercase text-[10px] tracking-widest transition-all flex items-center gap-2 ${
              activeTab === "following" ? "bg-[#FF9B9B] text-white shadow-lg shadow-[#FF9B9B]/30 scale-105" : "bg-white text-slate-400 hover:bg-slate-50 border border-slate-100"
            }`}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 mt-4">
          <AnimatePresence mode="wait">
            {loading ? (
              ...
              filteredUsers.map(u => (
                <motion.div key={u._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.2 }}>
                        ...
                        {u.lastDiaries && u.lastDiaries.length > 0 ? (
                          <div className="flex gap-2 h-20 items-center overflow-x-hidden">
                            {u.lastDiaries.map((diary, idx) => (
                              <div key={idx} className="w-14 h-18 shrink-0 rounded-xl p-2 flex flex-col justify-center items-center border border-white/40 shadow-sm transition-transform group-hover:scale-110" style={{ backgroundColor: diary.coverColor || '#A29BFE' }}>
                                <p className="text-[7px] font-black uppercase text-white drop-shadow-md text-center leading-tight break-words px-1">{diary.title || "Defter"}</p>
